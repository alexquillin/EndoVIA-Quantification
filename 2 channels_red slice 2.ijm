...
run("Duplicate...", "title=mask duplicate range=2");
selectWindow("mask");
...
saveAs("results", input_path+"Results/" + "Slice2_" + i + ".csv");
CTCF_sum = 0;
CTCF_sum_value = 0;
CTCF_sum_1 = 0;
CTCF_sum_value_1 = 0;
...
saveAs("results", input_path+"Results/" + "Slice1_" + i + ".csv");
...
for (i = 0 ; i < length; i++) {
	...
	nrow = (Table.size)-2;
		...
		for (j = 0; j < nrow; j++) {
			...
			setResult("slice_1", nb_row, CTCF_value);
			nb_row = nb_row+1;
	}
...
for (k = 0; k < length ; k++) {
	...
	for (l = 0; l < nrow; l++) {
		...
		setResult("slice_2", nb_row, CTCF_value);
		nb_row = nb_row +1;
...
Plot.create("Compared_Intensities", "slice_1", "slice_2");
Plot.add("Circle", Table.getColumn("slice_1", "Results"), Table.getColumn("slice_2", "Results"));
Plot.setStyle(0, "black,black,1.0,Circle");

Fit.doFit("Straight Line", Table.getColumn("slice_1", "Results"), Table.getColumn("slice_2", "Results"));
Fit.plot;

Plot.setAxisLabelSize(14.0, "plain");
Plot.setFontSize(14.0);
Plot.setXYLabels("slice_1", "slice_2");
